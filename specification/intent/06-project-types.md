...
Every project has a plaintext title, a state, a markdown description and different types of projects have different additional attributes described below.

A project can also reference prerequisite projects that have to be Enacted or Implemented before this project can be Enacted. A new prerequisite can be added by searching among projects that are not already direct or indirect prerequisites for this project and for which this project is not already a direct or indirect prerequisite. 

A user can "view all dependencies" \- open a tree graph where the central node is the opened project, nodes below it are its direct and indirect prerequisites and nodes above it are projects for which it is a direct or indirect prerequisite. Each node shows the title and state of its project. 
...
* Can have references to other Enacted Legislative or Institutional projects whose decisions this project will at least partially override, if Enacted. 

* Has to gain a majority or a supermajority of votes for its Current version to become an Enacted version. 

* Has to reference what problem metrics it aims to improve and how the proposed changes intend to improve each metric. 

* Does not have a budget. 

* Has to reference a ministerial domain. Although Legislative projects have no budget and no responsible institution (everyone has to follow the new laws), it is still useful for search convenience to categorize a Legislative project's ministerial domain. 

* Does not include executive orders.

* Its scope can only be set to levels on which legislation can be passed according to the country's constitution. 

### Executive projects

An **Executive project**:

* Also has to reference what problem metrics it aims to improve and how the proposed actions intend to improve each metric. 
...
* Has to have a budget defined for its whole duration, sufficient to successfully implement the project, along with optional stretch goals \- expected increases in impact or additional activities that can be carried out within the same period, if pledged funding reaches these higher goals. 
...
* Also has to reference what problem metrics it aims to improve and how the proposed changes intend to improve each metric. 

* Includes both legislation and executive orders. A typical example is every institution \- it needs a law that mandates it to decide and act on behalf of the voters in specified matters of public benefit and it also needs a budget to cover its fixed costs.

* Can have references to other Enacted Legislative or Institutional projects whose decisions this project will at least partially override, if Enacted. 

* Has to meet requirements of both legislation and an Executive project. E.g. it can also only be scoped to levels on which legislation can be passed, it has to have a date when its legal changes enter into force and it also has to have a budget and to reference a responsible ministerial domain or create a new ministerial domain and a new institution responsible for the created domain. 
