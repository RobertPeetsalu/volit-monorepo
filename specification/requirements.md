...
4. Displayed texts referenced in requirements should be highlighted in bold so their related requirements are visually easier to find. For example, behavior of a **Commit** button or validation rules for a **Voting deadline** datepicker.
...
6. If a component should behave differently depending on the user's role, the status of a displayed document or other condition, then first describe requirement to the common behavior in all cases and then below it make a sub-list of all cases describing the difference in behavior for each case and the conditions under which this case should be displayed. For example:
...
9. Duplication of requirements should be avoided in the code repository and even outside of it, like in task descriptions. Instead the file, chapter and item numbers of requirements should be referenced. For example, 
